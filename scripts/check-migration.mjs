...
check(Object.keys(weapons.weapons).length >= 6, 'shared weapon definitions cover the six campaign weapons');
check(missions.mission01.objectives.length === 9, 'mission 01 defines the nine-step objective chain');
check(
  missions.mission01.enemyPositions.length >= 26 && missions.mission01.enemyPositions.length <= 34,
  'mission 01 fields 26-34 hostiles'
);
check(movement.speeds.walk === 4.9 && movement.stance.standHeight === 1.72, 'shared movement data holds the single-player feel');
check(!loadout.campaign.killstreaksEnabled, 'campaign rules disable killstreaks');

check(legacyIndex.includes('id="campaignLaunch"'), 'legacy main menu owns the campaign launch card');
check(campaignIndex.includes('id="menuLink"'), 'campaign HUD links back to the shared main menu');
check(legacyBuild.includes('shared/movement.json'), 'legacy build generates movement constants from shared data');
check(legacyBuild.includes('shared/weapons.json'), 'legacy build generates weapon constants from shared data');
...
check(campaignCombat.includes('../../shared/missions.json'), 'campaign combat consumes shared missions');
check(campaignLevel.includes('../../shared/missions.json'), 'campaign level consumes shared missions');
check(campaignLevel.includes('../../shared/audio-params.json'), 'campaign level consumes shared audio params');
check(campaignPlayer.includes('../../shared/movement.json'), 'campaign player consumes shared movement');
