export async function runMenuSmoke(page, check) {
  ...
  check(
    !root.renderer.includes('检测中'),
    'menu background resolves WebGPU or its static fallback'
  );
  ...
  const prism = await page.evaluate(() => ({
    ...
    buttonOn: document
      .querySelector('[data-attachment="optic:prism_2_5"]')
      .classList.contains('on'),
    status: document.getElementById('attachmentStatus').textContent,
  }));
  check(
    prism.selected === 'prism_2_5' && prism.buttonOn && prism.status.includes('2.5×'),
    'clicking the 2.5x menu button visibly equips and persists the prism optic'
  );
  ...
  check(
    settings.sensitivity === 1.35 && settings.fov === 82,
    'global sensitivity and FOV update live'
  );
  ...
  check(
    await page.evaluate(() => !document.getElementById('singleMenu').hidden),
    'single-player entry owns the two existing maps'
  );
  await page.keyboard.press('Escape');
  ...
  check(
    campaignEntry.visible &&
      campaignEntry.mission.includes('鹰落') &&
      campaignEntry.hint.includes('不提供连杀奖励'),
