...
window.H = {
  ...
  lane(d) {
    ...
    for (let i = 0; i < 900; i++) {
      ...
      for (const h of [0.6, 1.2, 1.7]) {
        R.set(new THREE.Vector3(x, h, z), dir);
        R.far = d + 1.2;
        if (R.intersectObjects(worldSolid, false).length) {
          clear = false;
          break;
        }
      }
  ...
  async fidelity(n, d, ads) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      e.gunDropped = false;
      e.obj.visible = true;
      e.obj.position.set(L.tx, 0, L.tz);
      ...
      R.set(camera.position, fw);
      R.far = WEAPONS[player.weapon].range;
    ...
    return {
      ...
      dist: d,
      ads: !!ads,
      crosshairOnEnemy: onTarget,
      registered,
      ghostMisses: ghostMiss,
  ...
  async accuracy(n, d, ads) {
    ...
    return {
      ...
      shots,
      hit,
      pct: shots ? Math.round((hit / shots) * 100) : 0,
      adsEase: +player.adsEase.toFixed(2),
    };
  },
  wep(i) {
    switchWeapon(i);
  ...
  state() {
    return {
      ...
      ar: +player.armor.toFixed(0),
      k: G.kills,
      w: player.weapon,
      mag: WEAPONS[player.weapon].mag,
      ads: player.ads,
      adsK: +player.adsK.toFixed(2),
  ...
  epos() {
    return enemies.map((e) => ({
      n: e.name,
      x: +e.obj.position.x.toFixed(1),
      y: +e.obj.position.y.toFixed(2),
      z: +e.obj.position.z.toFixed(1),
      s: e.state,
      hp: e.hp,
    }));
  },
  rig() {
    if (!H._dp) {
      H._dp = damagePlayer;
      window.damagePlayer = (a, f) => {
        if (!window.__god) return H._dp(a, f);
      };
    }
  ...
  audit(minSize) {
    minSize = minSize === undefined ? 0.3 : minSize;
    ...
    for (const m of worldSolid) {
      ...
      for (let fx = 0.2; fx <= 0.8; fx += 0.3)
        for (let fz = 0.2; fz <= 0.8; fz += 0.3) {
          const px = box.min.x + s.x * fx,
            pz = box.min.z + s.z * fz;
          const y0 = Math.max(box.min.y, 0.3),
            y1 = Math.min(box.max.y, 1.7);
          if (y1 <= y0) {
            continue;
          }
          tries++;
  ...
  tunnel(lines, probeY) {
    ...
    for (let i = 0; i < lines; i++) {
      ...
      const sx = Math.cos(a) * (HALF - 1.5),
        sz = Math.sin(a) * (HALF - 1.5);
      const ex = -sx * 0.9,
        ez = -sz * 0.9;
          ...
          for (const y of ys) {
            ...
            if (h) {
              ...
              if (!seen.has(key)) {
                seen.add(key);
                hitsOut.push({
                  at: [+h.point.x.toFixed(1), +h.point.z.toFixed(1)],
                  y: y,
  ...
  inside(step) {
    ...
    for (let x = -HALF + 1; x <= HALF - 1; x += step)
      for (let z = -HALF + 1; z <= HALF - 1; z += step) {
        ...
        let enclosed = 0;
        for (const d of dirs) {
          R.set(new THREE.Vector3(x, 1.0, z), d);
          R.far = 40;
  ...
  async wtest() {
    ...
    for (let i = 0; i < WEAPONS.length; i++) {
      ...
      const h0 = G.hits,
        s0 = G.shots,
        sh0 = SHELLS.filter((s) => s.life > 0).length;
      player.fireCooldown = 0;
      player.pumpT = 0;
      player.boltT = 0;
      player.reloadT = 0;
      ...
      await H.frame();
      const flash = {
        sprite: +muzzleSprite.material.opacity.toFixed(2),
      ...
      out.push({
        ...
        fired: G.shots - s0,
        hit: G.hits - h0,
        dmgDealt: +(hp0 - e.hp).toFixed(0),
        shellsEjected: shellsNow - sh0,
        shellSide,
        flash,
        adsEase: adsK,
        adsFov: fov,
        wantFov: w.adsFov,
        scopeOpacity: scope,
        crosshairHidden: crossHidden,
        reloadStarted: reloading,
      });
      player.reloadT = 0;
      player.pumpT = 0;
      player.boltT = 0;
